...
The SDK batches spans in memory and the OTLP exporter retries 429/502/503/504 with backoff; it never writes to disk. Spans still queued during a long outage or a process crash are lost — the same trade-off every LLM observability SDK makes. For long-running production agents where that is not acceptable, run a local OpenTelemetry Collector with a persistent queue (`file_storage` write-ahead log, `sending_queue.storage`, `retry_on_failure.max_elapsed_time: 0`) and point `RESPAN_BASE_URL` at it; the Collector receiver must expose `traces_url_path: /api/v2/traces` because the SDK posts to `<base>/api/v2/traces`. A complete Collector template is in the `@respan/instrumentation-pi` README ("Delivery guarantees"). Never build a local journal into the application itself; bound the Collector's disk with `file_storage.max_size` and `queue_size` and alert on `otelcol_exporter_enqueue_failed_spans`.
