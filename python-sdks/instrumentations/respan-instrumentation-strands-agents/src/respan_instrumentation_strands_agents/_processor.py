...
def _parts_to_content(parts: Any) -> Any:
    ...
    for part in parsed_parts:
        ...
        if part_type == "text":
            ...
            content_blocks.append(serialize_value(part))
...
def _normalize_message(raw_message: Any, default_role: str) -> dict[str, Any] | None:
    ...
    return {"role": str(role), "content": normalized_content}
...
def _legacy_input_messages(span: ReadableSpan) -> list[dict[str, Any]] | None:
    ...
    for event_name, event_attributes in _get_events(span):
        if event_name == STRANDS_EVENT_SYSTEM_MESSAGE:
            ...
            event_name.startswith(STRANDS_EVENT_MESSAGE_PREFIX)
...
def _set_indexed_messages(
    ...
    for message_index, message in enumerate(messages):
        ...
        attrs[f"{indexed_prefix}.role"] = str(message.get("role") or "")
        content = message.get("content")
        attrs[f"{indexed_prefix}.content"] = _message_content_attr_value(content)
        tool_calls = _tool_calls_from_content(content)
        if tool_calls:
            attrs[f"{indexed_prefix}.tool_calls"] = json.dumps(tool_calls, default=str)
...
def _set_input_output_attrs(
    ...
    *,
    ...
    if input_messages:
        attrs[SpanAttributes.TRACELOOP_ENTITY_INPUT] = json.dumps(
            input_messages,
            default=str,
        )
    if output_messages:
        attrs[SpanAttributes.TRACELOOP_ENTITY_OUTPUT] = json.dumps(
            output_messages,
            default=str,
        )
...
def _enrich_agent_span(
    ...
    tool_definitions = _extract_tool_definitions(attrs=attrs)
    if tool_definitions:
...
def _enrich_task_span(
    ...
    operation_name = attrs.get(GEN_AI_OPERATION_NAME)
    entity_name = operation_name if isinstance(operation_name, str) else span.name
    _set_common_attrs(
        attrs=attrs,
...
def _enrich_tool_span(
    ...
    tool_input_payload = {
        ...
        "arguments": serialize_value(_safe_json_loads(tool_arguments)),
    }
    attrs[SpanAttributes.TRACELOOP_ENTITY_INPUT] = json.dumps(
        tool_input_payload,
        default=str,
...
def _strip_raw_attrs(attrs: dict[str, Any], log_type: str) -> dict[str, Any]:
    attrs = {
        ...
        and not any(key.startswith(prefix) for prefix in STRANDS_RAW_ATTR_PREFIXES_TO_STRIP)
    }
...
def enrich_strands_agents_span(span: ReadableSpan) -> None:
    ...
    if log_type == LOG_TYPE_AGENT:
        ...
        _enrich_chat_span(span=span, attrs=attrs)
    elif log_type == LOG_TYPE_TOOL:
        _enrich_tool_span(span=span, attrs=attrs)

    span._attributes = _strip_raw_attrs(attrs=attrs, log_type=log_type)
...
class StrandsAgentsSpanProcessor(SpanProcessor):
    ...
    def on_end(self, span: ReadableSpan) -> None:
        try:
            enrich_strands_agents_span(span=span)
        except Exception:
