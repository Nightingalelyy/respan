...
export class Respan {
  ...
  logBatchResults(
    ...
    const resolvedTraceId = otelTraceId ?? traceId ?? undefined; // auto-generated by buildReadableSpan if undefined

    // Determine parent for completion spans
    const parentSpanId = otelSpanId ?? undefined; // buildReadableSpan generates one for grouping span
    ...
    if (!otelSpanId) {
      // Generate a stable ID for the group span
      groupSpanId =
